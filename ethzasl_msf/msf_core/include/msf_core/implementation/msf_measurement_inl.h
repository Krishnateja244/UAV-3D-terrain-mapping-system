...
void MSF_MeasurementBase<EKFState_T>::CalculateAndApplyCorrection(
  ...
  //       2, "R matrix or measurement noise:"<< R_delayed); 
  // MSF_WARN_STREAM_THROTTLE(
  //       2, "p matrix :"<< P);
  // printing Kalman gain for position
  MSF_WARN_STREAM_THROTTLE(
        2, "x-gain :"<< K(0,0));
  MSF_WARN_STREAM_THROTTLE(
        2, "y-gain :"<< K(1,1));
  MSF_WARN_STREAM_THROTTLE(
        2, "z-gain :"<< K(2,2));
  // std::cout<<"position-x: "<<K(0, 0)<<std::endl;
  // std::cout<<"position-y: "<<K(1, 1)<<std::endl;
  // std::cout<<"position-z: "<<K(2, 2)<<std::endl;
  //ros::init(argc, argv, "kalman_gain");
  //ros::NodeHandle nh("msf_core");
  // ros::NodeHandle n;
  // ros::Publisher pubkalman = n.advertise < sensor_fusion_comm::DoubleArrayStamped
  //       > ("kalmangain", 100);
  // sensor_fusion_comm::DoubleArrayStamped msg_gain;
  // // msg_gain.header.stamp = ros::Time(state->time);
  // msg_gain.data[0] = K(0, 0);
  // msg_gain.data[1] = K(1, 1);
  // msg_gain.data[2] = K(2, 2);
  // pubkalman.publish(msg_gain);
